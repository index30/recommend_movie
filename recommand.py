...
class Reccomend:
    ...
    # 類似ユーザを元に映画推薦
    def user_recommand(i_id, u_id, dic, mov):
        i_dic = dic[str(i_id)]
        u_dic = dic[str(u_id)]
...
if __name__ == "__main__":
    ...
    with open(data_path, 'r') as d, open(item_path, 'r') as i, open(user_path, 'r') as u:
        ...
        Main.user_recommand(input_id, sim_id, data_dic, mov_list)
        print("")
        print("Item-base recommend...")
        mat = Item_base.item_mat(data_dic)
        Main.item_recommend(data_dic, input_id, mat, mov_list)
